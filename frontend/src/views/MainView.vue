    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">模拟实例初始化</span>
        </div>
...
const handleNewProject = async () => {
  ...
  try {
    ...
    ontologyProgress.value = { message: '上传文件并生成本体...' }
    addLog('Step 01.A: Uploading files & generating ontology...')
    ...
    if (res.success) {
      ...
      addLog(`Ontology generated for ${res.data.project_id}`)
  ...
  try {
    ...
    if (res.success) {
      ...
      if (res.data.status === 'ontology_generated' && !res.data.graph_id) {
        ...
        ['graph_completed', 'personas_generating', 'personas_completed'].includes(res.data.status) &&
        res.data.graph_id
      ) {
...
const startBuildGraph = async () => {
  try {
    currentPhase.value = 1
    buildProgress.value = { progress: 0, message: '启动构建...' }
    addLog('Step 01.B: Initiating graph build...')
...
const startGraphPolling = () => {
  addLog('Started polling graph data (real-time draw)...')
  fetchGraphData()
  graphPollTimer = setInterval(fetchGraphData, 3000)
}
...
const loadGraph = async (graphId) => {
  ...
  addLog(`Loading full graph: ${graphId}`)
  try {
...
const handleGoPersonas = () => {
  router.push({ name: 'Personas', params: { projectId: currentProjectId.value } })
}
...
.panel-wrapper { height: 100%; overflow: hidden; transition: width 0.4s cubic-bezier(0.25, 0.8, 0.25, 1), opacity 0.3s ease, transform 0.3s ease; will-change: width, opacity, transform; }
